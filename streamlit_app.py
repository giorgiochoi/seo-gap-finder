        ...
        with st.spinner("🕵️ Searching for top competitor via Serper.dev..."):
            try:
                ...
                if 'organic' not in search_data or not search_data['organic']:
                    st.error("Could not find any search results for that keyword. Try another.")
                    st.stop()
                
                comp_url = search_data['organic'][0]['link']
                st.info(f"Targeting Competitor: {comp_url}")

                # STEP 2: Scrape both sites via Firecrawl
                with st.spinner("📄 Scraping content..."):
                    user_data = firecrawl.scrape_url(user_url, params={'formats': ['markdown']})
                    comp_data = firecrawl.scrape_url(comp_url, params={'formats': ['markdown']})
                ...
                with st.spinner("🤖 Claude is analyzing the gap..."):
                    prompt = (
                        ...
                        f"USER SITE CONTENT: {user_data['markdown'][:8000]}\n\n"
                        f"COMPETITOR CONTENT: {comp_data['markdown'][:8000]}\n\n"
...
if st.session_state.report_ready:
    ...
    st.write("Enter your email below to receive a deep-dive execution plan generated by our AI.")
    ...
    with st.form("lead_capture"):
        ...
        if submit_lead:
            if "@" not in email:
                ...
                st.balloons()
                st.success("Success! Check your inbox in a few minutes.")
